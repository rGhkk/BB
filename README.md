...
### 2. "element not interactable" 오류
**원인**: 요소를 찾았지만 클릭하거나 입력할 수 없는 상태 (다른 요소에 가려져 있거나 아직 활성화되지 않음)

**해결** (v1.1.0에서 자동 수정됨):
- 스크립트가 자동으로 스크롤 및 JavaScript 폴백 시도
- 실패 시 수동 입력 안내 메시지 표시
- 페이지가 완전히 로드될 때까지 기다린 후 스크립트 실행

### 3. "프롬프트 입력창을 찾을 수 없습니다" 오류
...
- **v1.1.0** (2026-01-05): element not interactable 오류 수정
  ...
  - 자동 스크롤로 요소를 뷰포트에 표시
  - JavaScript 폴백 메커니즘 추가 (일반 클릭 실패 시)
  - 페이지 로딩 대기 시간 증가 (3초 → 5초)
  - 입력 이벤트 트리거 추가
  - 에러 발생 시 수동 입력 안내 개선
